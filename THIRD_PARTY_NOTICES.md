...
- `mo:sha2`: https://github.com/research-ag/sha2 — Apache-2.0。`apps/01_creator_proof_registry`がon-chain SHA-256 commitment verificationのために、`apps/02_merkle_anchor`がon-chain Merkle proof verificationのために、`apps/03_license_marketplace`がpayment intent memoのために、`apps/05_usage_metered_saas`がinvoice payment memoのために`mops install`時にdependencyとして取得します（source再配布はしていません）。
